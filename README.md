...
# Functionality
![MSIRGB](https://i.imgur.com/3BPc9xP.png)

**The program allows you to set up to 8 different colours for the LEDs to switch between.** The LED controller will switch between those colours in a specific pattern, with an interval between each switch, called 'step duration', and which can be changed. The pattern is as follows: 8, (5, 6, 3, 4, 1, 2), 1, (4, 3, 6, 5, 8, 7), repeat, where each digit corresponds to a colour number (the rectangles at the top, from left to right, represent each colour from 1-8). This function mirrors the hardware implementation, i.e. it merely provides you with a way to use the built-in functions of the MB's LED controller. **Scripts allow you to change between any number of colours.** There is a limit to how many colours the chip understands, which is (only) 3,375. Because of that, there is a limit to how smooth a colour transition can be (see the hue wheel effect, for example). Additionally, colours are approximated, i.e. the colour you see in the colour picker is not necessarily exactly the colour the LEDs display (though, it's close enough). This applies for scripts as well - there is no magic there.
